...
public sealed partial class FitnessDevice
{
    ...
    {
        var characteristic = _characteristics.FirstOrDefault(c => c.Id == FitnessDeviceGuids.FitnessMachineControlPointCharacteristicUuid)
            ?? throw new InvalidOperationException("FTMS Control Point characteristic not found.");

        _ = await characteristic.WriteAsync(payload);
    }
}
